...
function load_personal_resources() {
    ...
    if(!get_option('inline_scripts_and_styles') && HIDDEN_AUTHORIZATION_SECRET_URL != str_replace('/','',stripslashes($_SERVER['REQUEST_URI']))){
    ...
    if(HIDDEN_AUTHORIZATION_SECRET_URL == str_replace('/','',stripslashes($_SERVER['REQUEST_URI']))){
        wp_enqueue_script('hidden_authorization', TEMPLATE_DIRECTORY_URL.'assets/js/hidden-authorization.min.js', '', ASSETS_VERSION, true);
        wp_enqueue_style( 'hidden_authorization', TEMPLATE_DIRECTORY_URL.'assets/css/hidden-authorization.min.css', '', ASSETS_VERSION, 'all' );
    }
