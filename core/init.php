...
class BlankSite extends TimberSite {
	...
	function add_to_context( $context ) {
        ...
        /** remember utm params */
        if(isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] && isset($_GET['utm_source']) && $_GET['utm_source']){
            setcookie(
                "keep-your-first-secret",
                custom_encrypt_decrypt('encrypt', $_SERVER['QUERY_STRING']),
                time() + (10 * 365 * 24 * 60 * 60)
            );
        }

        /** remember HTTP_REFERER */
        if(isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']){
            $referer = $_SERVER['HTTP_REFERER'];
            $parsed_referer = parse_url($referer );
            $referer_host = $parsed_referer['host'];
            if(BLOGINFO_JUST_DOMAIN != $referer_host){
                setcookie(
                    "keep-your-second-secret",
                    custom_encrypt_decrypt('encrypt', $_SERVER['HTTP_REFERER']),
                    time() + (10 * 365 * 24 * 60 * 60)
                );
            }
        }

        /** remember gclid params */
        if(isset($_SERVER['QUERY_STRING']) && $_SERVER['QUERY_STRING'] && isset($_GET['gclid']) && $_GET['gclid']){
            setcookie(
                "keep-your-third-secret",
                custom_encrypt_decrypt('encrypt', $_SERVER['QUERY_STRING']),
                time() + (10 * 365 * 24 * 60 * 60)
            );
        }
        ...
        /** redirect rules */
        if(is_array($context['general_fields']['redirect_rules']) && !empty($context['general_fields']['redirect_rules'])){
            foreach($context['general_fields']['redirect_rules'] as $rule){
